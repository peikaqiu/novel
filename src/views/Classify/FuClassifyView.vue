<template>
  <div>
    <div v-if="isAjax">Loding</div>
    <div class="classify" ref="classifyLoop" v-else>
      <div class="classify_box">
        <div class="header">
          <div class="header_container">
            ...
            <div class="sex">
              <div class="male"><span class=active>男生</span></div>
              <div class="female"><span @click=" setTofemale">女生</span></div>
            </div>
        ...
        <div class="classify_contain">
          <div class="fuclassify" v-for="(item,index) in getFuClassify.male" :key="index"
               v-show="getZiClassify.male[index].mins.length>0">
            <div class="futitle" @click="getKeyeord(item.name)">
              {{item.name}}
              <span class="count">共{{item.bookCount}}本作品</span>
              <i class="iconfont icon-arrow-right"></i>
            </div>
            <div class="ziclassify" v-for="(items,index) in getZiClassify.male" :key="index"
...
<script>
  ...
  export default{
    ...
    methods: {
      ...
      setTofemale(){
        this.$router.push({path: '/ZiClassify'})
      },
      getKeyeord(keyword){
        console.log(keyword);
        this.SET_TITLE(keyword)
        this.getClassifyContent({axios:this.axios, gender:'male', major:keyword, minor:''})
        this.$router.push({path:'/ClassifyContent'})
      },
      getsonKeyword(keyword,e){
        console.log(keyword,e);
        this.SET_TITLE(e.target.innerHTML)
        this.getClassifyContent({axios:this.axios, gender:'male', major:keyword, minor:e.target.innerHTML})
        this.$router.push({path:'/ClassifyContent'})
      }
    },
    created(){
